...
namespace MvcContrib.UnitTests.UI.Html
{
	...
	public class HtmlGridTester
	{
		...
		public void Setup()
		{
			...
			viewContext = new ViewContext(context, new RouteData(), mocks.DynamicMock<IController>(), "index", "", new Hashtable(), null);
			helper = new HtmlHelper(viewContext);
		...
		{
			((Hashtable)viewContext.ViewData).Add(key, value);
		}
		...
		public void Should_render_empty_table_when_collection_is_empty()
		{
			people.Clear();
			string expected = "<table class=\"grid\"><tr><td>There is no data available.</td></tr></table>";
			helper.Grid<Person>("people", column => { column.For(p => p.Name); column.For(p => p.Id); });
		...
		public void Should_render_empty_table_with_custom_message()
		{
			...
			helper.Grid<Person>((string)null, new Hash(empty => "Test"), column => { column.For(p => p.Name); column.For(p => p.Id); });
			Assert.That(Writer.ToString(), Is.EqualTo(expected));
		}
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Name).Header(() => Writer.Write("<td>TEST</td>")); column.For(p => p.Id); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.DateOfBirth).Formatted("{0:ddd}"); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Id.ToString() + "-" + p.Name, "Test"); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Id.ToString() + "-" + p.Name); });
		...
		{
			helper.Grid<Person>("people", column => { column.For("Name").Do(s => Writer.Write("<td>Test</td>")); });
		...
		public void With_anonymous_type()
		{
			...
			helper.Grid<object>("test", column => { column.For("Name"); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Name); column.For(p => p.Id).CellCondition(p => false); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Name); column.For(p => p.Id).ColumnCondition(() => false); });
		...
		public void Should_encode()
		{
			...
			helper.Grid<Person>("people2", column => { column.For(p => p.Name); });
			string expected = "<table class=\"grid\"><thead><tr><th>Name</th></tr></thead><tr><td>Jeremy&amp;</td></tr></table>";
			Assert.That(Writer.ToString(), Is.EqualTo(expected));
		}
		...
		{
			AddToViewData("people2", new List<Person> { new Person { Name = "Jeremy&" } });
			helper.Grid<Person>("people2", column => { column.For(p => p.Name).DoNotEncode(); });
			string expected = "<table class=\"grid\"><thead><tr><th>Name</th></tr></thead><tr><td>Jeremy&</td></tr></table>";
			Assert.That(Writer.ToString(), Is.EqualTo(expected));
		}
		...
		{
			helper.Grid<Person>(new List<Person> { new Person { Id = 1 } }, column => { column.For(p => p.Id); });
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Name); column.For(p => p.Id); column.RowEnd(person =>
			                                                                                                        	{
		...
		{
			helper.Grid<Person>("people", column => { column.For(p => p.Name); column.For(p => p.Id); column.RowStart(p =>
			                                                                                                          	{
