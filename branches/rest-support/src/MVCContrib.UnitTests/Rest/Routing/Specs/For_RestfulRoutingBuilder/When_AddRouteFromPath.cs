...
{
	[TestFixture]
	public class When_AddRouteFromPath : RestfulRoutingFixture
	{
		protected override void SetUp()
		{
			_restfulRoutingBuilder.ForAnyController().FromPath("admin").Register();
		...
		public void Then_CreateUrl_To_Controller_Is_The_Default_Url()
		{
			...
			Assert.That(url, Is.EqualTo("/admin/test").IgnoreCase);
		}
		...
		public void Then_CreateUrl_To_Delete()
		{
			...
			Assert.That(url, Is.Not.Null);
			Assert.That(url, Is.EqualTo("/admin/test/123/delete").IgnoreCase);
		}
		...
		public void Then_CreateUrl_To_Edit()
		{
			...
			Assert.That(url, Is.Not.Null);
			Assert.That(url, Is.EqualTo("/admin/test/123/edit").IgnoreCase);
		}
		...
		public void Then_CreateUrl_To_New()
		{
			...
			Assert.That(url, Is.Not.Null);
			Assert.That(url, Is.EqualTo("/admin/test/new").IgnoreCase);
		}
		...
		public void Then_CreateUrl_To_Show_Is_Default_Url_For_Entity()
		{
			...
			Assert.That(url, Is.Not.Null);
			Assert.That(url, Is.EqualTo("/admin/test/123").IgnoreCase);
		}

		[Test]
		public void Then_Default_Rules_Are_Not_Added()
		{
			Assert.That(With.GetRequest.To("/test").Match(), Is.Null);
			Assert.That(With.GetRequest.To("/test/index").Match(), Is.Null);
			Assert.That(With.GetRequest.To("/test/123").Match(), Is.Null);
			Assert.That(With.GetRequest.To("/test/123/show").Match(), Is.Null);
			Assert.That(With.GetRequest.To("/test/123/edit").Match(), Is.Null);
			Assert.That(With.GetRequest.To("/test/123/delete").Match(), Is.Null);
		}
		...
		{
			RouteData match = With.DeleteRequest.To("/admin/test/123").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test/index").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test/123/delete").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test/123/edit").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test/123/show").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test/123").Match();
		...
		{
			RouteData match = With.GetRequest.To("/admin/test").Match();
		...
		public void
			...
			()
		{
			RouteData match = With.PostRequest.To("/admin/test/123")
		...
		{
			RouteData match = With.PostRequest.To("/admin/test").Match();
		...
		{
			RouteData match = With.PutRequest.To("/admin/test/123").Match();
		...
		public void Then_The_First_Fragment_Of_Each_Route_Is_A_Controller_Fragment()
		{
			...
			_restfulRoutingBuilder.RuleSets.ForEach(
				ruleSet =>
				ruleSet.Rules.ForEach(
					rule => Assert.That(rule.Fragments.Skip(1).First().Name, Is.EqualTo("controller"))));
		}

		[Test]
		public void Then_Will_Not_Match_A_Get_Request_To_The_Controller_Name_And_String_Id()
		{
			RouteData match = With.GetRequest.To("/admin/test/goose").Match();
