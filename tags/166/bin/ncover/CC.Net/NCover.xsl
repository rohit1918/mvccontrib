<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
  ...
  <!-- created by Yves Lorphelin, largely inspired by the nunitsumary.xsl (see nantcontrib.sourceforge.net)-->
	...
	<xsl:template name="ModuleSummaryDetail">
		...
		<xsl:variable name="visited" select="count(./class/method/seqpnt[not(@vc='0')] )" />
		<td width="30%">
			<div class="assembly">
	...
	<xsl:template name="module">
		...
		<xsl:variable name="visitedMod" select="count(./class/method/seqpnt[not(@vc='0')] ) div $totalMod * 100" />
	</xsl:template>
	<!-- Class Summary -->
	<xsl:template name="ClassSummary">
		...
		<xsl:variable name="visited" select="count(//seqpnt[(parent::method/parent::class/@name=$class) and (parent::method/parent::class/parent::module/@assembly=$module) and (not(@vc='0'))] )" />
	...
	<xsl:template name="Methods">
		...
		<xsl:for-each select="//method[(parent::class/@name = $class) and (parent::class/parent::module/@assembly=$module)]">
			...
			<xsl:variable name="visited" select="count(./seqpnt[not(@vc='0')])" />
	...
	<xsl:template name="seqpnt">
		...
		<xsl:param name="class" />
		<xsl:param name="id" />
		<table cellpadding="3" cellspacing="0" border='1' bordercolor="black" style="display: block;">
			<xsl:attribute name="id">
