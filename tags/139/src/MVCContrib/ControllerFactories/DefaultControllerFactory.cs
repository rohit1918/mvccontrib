...
namespace MvcContrib.ControllerFactories
{
	public class DefaultControllerFactory : System.Web.Mvc.DefaultControllerFactory
	{
		...
		protected override Type GetControllerType(string controllerName)
		{
			...
			return (Type)_typeCache[controllerName + (controllerName.EndsWith("Controller") ? "" : "Controller")];
		}
		...
		protected void EnsureCacheInitialised()
		{
			if(_typeCache == null)
			{
				lock(_lock)
				{
					if(_typeCache == null)
					{
						var cache = new Hashtable(StringComparer.OrdinalIgnoreCase);
						...
						foreach(var assembly in assemblies)
						{
							foreach(var type in assembly.GetTypes())
							{
								if(IsController(type))
								{
									...
									{
										throw new Exception(
											string.Format("Multiple controllers found with the name '{0}'", type.Name));
									}
